...
styles = [
    {"color": "c", "marker": "+", "markeredgecolor": "c", "markerfacecolor": "None"},
    {"color": "g", "marker": "v", "markeredgecolor": "g", "markerfacecolor": "None"},
    {"color": "b", "marker": "*", "markeredgecolor": "b", "markerfacecolor": "None"},
    {"color": "k", "marker": "o", "markeredgecolor": "k", "markerfacecolor": "None"},
    {"color": "r", "marker": "s", "markeredgecolor": "r", "markerfacecolor": "None"},
    {"color": "m", "marker": "D", "markeredgecolor": "m", "markerfacecolor": "None"},
    {"color": "k"},
    {"color": "y"},
    {"color": "k"},
    {"color": "c"},
...
def beautify(xmin=None):
    ...
    ymin, ymax = plt.ylim()
    ybnd = max(1.0 / ymin, ymax)
    plt.ylim(1.0 / ybnd, ybnd)
    if ybnd < 100:
...
def annotate(entry0, entry1, dim, minfvalue=1e-8, nbtests=1):
    ...
    plt.text(annotcoord[0], annotcoord[1], txt, horizontalalignment=ha, verticalalignment=va, transform=trans)

    # ranksum test
    line0 = np.power(data0, -1.0)
    line0[np.isnan(line0)] = -entry0.finalfunvals[np.isnan(line0)]
    line1 = np.power(data1, -1.0)
    ...
    if nbstars > 0:
        xstars = annotcoord[0] * np.power(incrstars, np.arange(1.0, 1.0 + nbstars))
...
def main(dsList0, dsList1, minfvalue=1e-8, outputdir=""):
    ...
    dimensions = testbedsettings.current_testbed.dimensions_to_display
    dimension_index = dict([(dimensions[i], i) for i in range(len(dimensions))])  # noqa: F841
    ...
    for func in set.intersection(set(dictFun0), set(dictFun1)):
        ...
        for i, dim in enumerate(dimensions):
            ...
            idx = (data[0][:, 1] <= 3 * np.median(entry0.maxevals)) * (data[1][:, 1] <= 3 * np.median(entry1.maxevals))
        ...
        for i, dim in enumerate(dimensions):
            ...
            idx = (data[algstoppedfirst][:, 2] == 0.0) * (dataofinterest[:, 2] > 0.0)
            idx[tmp] = True
            ...
            ydata = np.power(10, np.log10(ybnd) * (dataofinterest[idx, 2] - offset * (5 - i) * np.log10(ymax / ymin) / np.abs(np.log10(ybnd))))

            ls = "-"
            if dataofinterest[idx, 0][0] < fvalueswitch[dim]:
                ls = "--"

            tmp = plt.plot([dataofinterest[idx, 0][0]] * 2, (tmpy, ydata[0]), **styles[i])
            plt.setp(tmp, ls=ls, marker="")
            tmp = plt.plot(
                (dataofinterest[idx, 0][0],),
                (ydata[0],),
                marker="D",
                ls="",
                color=styles[i]["color"],
