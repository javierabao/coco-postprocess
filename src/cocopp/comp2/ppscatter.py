...
offset = 0.0  # 0.02 offset provides a way to move away the box boundaries to display the outer markers fully, clip_on=False is more effective
...
def figure_caption(for_html=False):
    ...
    if for_html:
        caption = htmldesc.getValue("##bbobppscatterlegend" + testbedsettings.current_testbed.scenario + "##")
    else:
...
def beautify():
    ...
    maxbnd = maxbnd ** (1 + 11.0 * offset / (numpy.log10(float(maxbnd) / minbnd)))
    ...
    for grid_line_pos in [
        ...
        plt.plot(grid_line_pos[0], grid_line_pos[1], ls="-", lw=1, color="lightgray")
...
def main(dsList0, dsList1, outputdir, settings):
    ...
    for f in funcs:
        ...
        if isinstance(targets, pproc.RunlengthBasedTargetValues):
            text = str(len(targetlabels)) + " target RLs/dim: " + targetlabels[0] + ".." + targetlabels[len(targetlabels) - 1] + "\n"
            text += "   from " + testbedsettings.current_testbed.reference_algorithm_filename
        else:
            text = str(len(targetlabels)) + " targets: " + targetlabels[0] + ".." + targetlabels[len(targetlabels) - 1]
        ...
        if len(set(num_of_instances_alg0)) == 1 and len(set(num_of_instances_alg1)) == 1:
            ...
            text = text.rstrip(", ")
            text += " instances"
        plt.text(0.01, 0.98, text, horizontalalignment="left", verticalalignment="top", transform=plt.gca().transAxes, size=0.6 * 17)
        ...
        a = plt.gca()
        a.set_xlabel("log10(ERT of %s)" % dsList0[0].algId[:18], fontsize=0.85 * genericsettings.rcfont["size"])
        a.set_ylabel("log10(ERT of %s)" % dsList1[0].algId[:18], fontsize=0.85 * genericsettings.rcfont["size"])
