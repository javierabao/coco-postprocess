...
def plotLogAbs(dsList0, dsList1, dim, targetValuesToReach):
    ...
    for j in range(len(targetValuesToReach)):
        ...
        for func in evals0:
            ...
            try:
                ...
                with warnings.catch_warnings():
                    warnings.filterwarnings("ignore", message=r"invalid value encountered in", category=RuntimeWarning)
        ...
        if len(x) == 0:
            ...
            y2 = numpy.hstack([tmp / float(n), numpy.repeat(numpy.arange(tmp + 1, n - tmp2) / float(n), 2), (n - tmp2) / float(n)])
...
def plotLogRel(indexEntries0, indexEntries1, isByInstance=True):
    ...
    while curevals < maxevals:
                ...
                for i, entry in enumerate((i0, i1)):
                    ...
                    for k in dictinstance:
                        for j in entry.evals:
                            if j[0] <= curDf[k]:
                                break
                        ERT[i][k] = computeERT(j[list(1 + i for i in dictinstance[k])], entry.maxevals[list(i for i in dictinstance[k])])
        ...
        if not x:
            ...
            xbound = max(abs(numpy.floor(numpy.log10(x[0]))), abs(numpy.ceil(numpy.log10(x[-1]))))
            x2 = numpy.hstack([10.0 ** (-xbound), numpy.repeat(x, 2), 10.0**xbound])
            # maxEvalsF: used for the limit of the plot.
            y2 = numpy.hstack(
                [
                    tmp / float(nn),
...
def main(dsList0, dsList1, dim, targetsOfInterest=None, outputdir="", info="default"):
    ...
    handles = plotLogAbs(dsList0, dsList1, dim, targetsOfInterest)
    ...
    plt.text(0.98, 0.02, text, horizontalalignment="right", transform=plt.gca().transAxes)
