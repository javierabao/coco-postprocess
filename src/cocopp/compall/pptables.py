...
def get_table_caption():
    ...
    table_caption_rest = (
        ...
        + (
            ...
            r"""shown only when less than 10 percent were worse and the ERT was better.""".format(significance_vs_ref_symbol)
            if not (
                testbedsettings.current_testbed.reference_algorithm_filename == ""
                or testbedsettings.current_testbed.reference_algorithm_filename is None
            )
            else ""
        )
        + r""" Best results are printed in bold.
        """
    )
    ...
    * table width...
    ...
    for df in sorted(dict_data):
        ...
        for n in sorted(dict_data[df].keys()):
            ...
            if len(entries) > 1:
                print(entries)
                txt = "There is more than a single entry associated with folder %s on %d-D f%d." % (sorted_algs[n], df[0], df[1])
                raise Exception(txt)
            ...
            for i, e in enumerate(evals):
                ...
                if succ.any():
                    tmp = toolsstats.drawSP(
                        ec[succ], entry.maxevals[succ == False], [10, 50, 90], samplesize=entry.bootstrap_sample_size(samplesize)
                    )[0]
                    tmpdisp.append((tmp[-1] - tmp[0]) / 2.0)
                else:
        ...
        for i, erts in enumerate(algerts):
            ...
            for j, ert in enumerate(erts):  # algi targetj
                tmp.append(i in tmptop[j] or (refalgentries and nalgs > 7 and algerts[i][j] <= 3.0 * refalgert[j]))
        ...
        significance_versus_others, best_alg_idx = significance_all_best_vs_other(algentries, targets, best_alg_indices(algerts, algmedfinalfunvals))
        # Create the table
        table = []
        tableHtml = []
        spec = r"@{}c@{}|*{%d}{@{\,}r@{}X@{\,}}|@{}r@{}@{}l@{}" % (len(targets_of_interest))  # in case StrLeft not working: replaced c@{} with l@{ }
        spec = r"@{}c@{}|*{%d}{@{}r@{}X@{}}|@{}r@{}@{}l@{}" % (len(targets_of_interest))  # in case StrLeft not working: replaced c@{} with l@{ }
        extraeol = []
        ...
        if with_table_heading:
            ...
            table.append([r"\multicolumn{%d}{@{\,}c@{\,}}{{\textbf{%s}}}" % (2 * len(targets_of_interest) + 2, header)])
            extraeol.append("")
        ...
        if function_targets_line is True or (function_targets_line and df[1] in function_targets_line):
            if isinstance(targets_of_interest, pproc.RunlengthBasedTargetValues):
                ...
                if testbed.name in [testbedsettings.suite_name_bi, testbedsettings.suite_name_bi_ext, testbedsettings.suite_name_bi_mixint]:
                    curline = [r"$\Df$"]
                else:
                    curline = [r"$\Delta f_\mathrm{opt}$"]
                counter = 1
                for t in targets:
                    curline.append(r"\multicolumn{2}{@{\,}l@{\,}}{%s}" % writeFEvals2(t, precision=1, isscientific=True))
                    counter += 1
        ...
        if refalgentries:
            if isinstance(targets_of_interest, pproc.RunlengthBasedTargetValues):
                ...
                for i in range(len(refalgert[:-1])):
                    ...
                    if temp[-2] == "0":
                        temp = temp[:-2] + temp[-1]
                    curline.append(r"\multicolumn{2}{@{}c@{}}{\textit{%s}:%s \quad}" % (temp, writeFEvalsMaxPrec(refalgert[i], 2)))
                ...
                if temp[-2] == "0":
                    temp = temp[:-2] + temp[-1]
                curline.append(r"\multicolumn{2}{@{}c@{}|}{\textit{%s}:%s }" % (temp, writeFEvalsMaxPrec(refalgert[-1], 2)))
                ...
                for i in refalgert[:-1]:
                    curline.append(r"\multicolumn{2}{@{}c@{}}{%s \quad}" % writeFEvalsMaxPrec(i, 2))
                    curlineHtml = [item.replace("REPLACE%d" % counter, writeFEvalsMaxPrec(i, 2)) for item in curlineHtml]
                    counter += 1
                curline.append(r"\multicolumn{2}{@{}c@{}|}{%s}" % writeFEvalsMaxPrec(refalgert[-1], 2))
        ...
        for i, alg in enumerate(algnames):
            ...
            zipToEnumerate = (
                zip(algerts[i], algdisp[i], isBoldArray[i], algtestres[i]) if refalgentries else zip(algerts[i], algdisp[i], isBoldArray[i])
            )
            ...
            for j, tmp in enumerate(zipToEnumerate):  # j is target index
                ...
                if numpy.isfinite(ert) and refalgentries and numpy.isinf(refalgert[j]):
                    ...
                    curline.append(r"\multicolumn{2}{%s}{%s}%s" % (alignment, tableentry, str_significance_subsup))
