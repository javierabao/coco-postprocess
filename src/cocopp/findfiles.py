...
def is_recognized_repository_filetype(filename):
    return os.path.isdir(filename.strip()) or filename.find(".tar") > 0 or filename.find(".tgz") > 0 or filename.find(".zip") > 0
...
def is_recognized_repository_filetype2(filename):
    ...
    if os.path.isdir(n):
        return False
    return n.endswith(".tgz") or n.endswith(".zip") or n.endswith(".tar.gz") or n.endswith(".tar")
                    ...
                    with zipfile.ZipFile(directory, "r") as zip_ref:
                        ...
                        if "win32" in sys.platform:
                            ...
                            if len(dir_name) + longest_file_length > 259:
                                warnings.warn("Some of the files can probably not be extracted " + 'from "%s" as the path is too long.' % directory)
