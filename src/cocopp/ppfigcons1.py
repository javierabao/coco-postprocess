...
def beautify(dim, axesLabel=True):
    ...
    axisHandle.set_yticks([10.0**i for i in range(0, logyend)])
    ...
    plt.vlines(dim, 0, upper_bound, lw=2, linestyles="--", alpha=0.5, colors=["blue"], label="m = dim")
    plt.vlines(int(dim * 1.5), 0, upper_bound, lw=2, linestyles="--", alpha=0.5, colors=["green"], label="m(active) = dim")
...
def plot_a_bar(
    ...
    if fill_color is not None:
        plt.fill_between(
            [x[0], xm[0], x[0], x[1], xm[1], x[1], x[0]], [y[0], y[1], y[2], y[2], y[1], y[0], y[0]], color=fill_color, alpha=1 - fill_transparency
        )
    plot_cmd([x[0], xm[0], x[0], x[1], xm[1], x[1], x[0]], [y[0], y[1], y[2], y[2], y[1], y[0], y[0]], markersize=0, **styles2)
    styles2["linewidth"] = 0
    plot_cmd([x[0], x[1], x[1], x[0], x[0]], [y[0], y[0], y[2], y[2], y[0]], **styles2)
    styles2["linewidth"] = 2  # median
    plot_cmd([x[0], x[1]], [y[1], y[1]], markersize=0, **styles2)
...
def plot(dsList, valuesOfInterest=None, styles=styles):
        ...
        if len(succ) > 0:
                ...
                for icons, cons in enumerate(constraints):
                    ...
                    if dsList[icons].detSuccesses([valuesOfInterest()[i_target]])[0]:
                        ...
                        y = toolsstats.drawSP_from_dataset(
                            dsList[icons], valuesOfInterest()[i_target], [25, 50, 75], genericsettings.simulated_runlength_bootstrap_sample_size
                        )[0]
                        rec_width = 1.1  # box ("rectangle") width
                        rec_taille_fac = 0.3  # notch width parameter
                        r = rec_width ** ((1.0 + i_target / 3.0) / 4)  # more difficult targets get a wider box
                        styles2 = {}
                        ...
                        plt.plot([x[0], xm[0], x[0], x[1], xm[1], x[1], x[0]], [y[0], y[1], y[2], y[2], y[1], y[0], y[0]], markersize=0, **styles2)
                        styles2["linewidth"] = 0
                        plt.plot([x[0], x[1], x[1], x[0], x[0]], [y[0], y[0], y[2], y[2], y[0]], **styles2)
                        styles2["linewidth"] = 2  # median
                        plt.plot([x[0], x[1]], [y[1], y[1]], markersize=0, **styles2)
            ...
            for i, n in enumerate(data[:, 0]):
                ...
                if constraints[j + 1] == data[i + 1, 0]:
                    res.extend(
                        plt.plot(
                            data[i : i + 2, 0], data[i : i + 2, 1] / ds.dim**ynormalize_by_dimension, markersize=0, clip_on=True, **styles[i_target]
                        )
                    )
            # plot only marker
            lw = styles[i_target].get("linewidth", None)
            styles[i_target]["linewidth"] = 0
            res.extend(plt.plot(data[:, 0], data[:, 1] / ds.dim**ynormalize_by_dimension, markersize=20, clip_on=True, **styles[i_target]))
            # restore linewidth
            if lw:
    ...
    if 1 < 3:  # maxevals
        ...
        res.extend(
            plt.plot(
                data[:, 0],
                maxevals / ds.dim**ynormalize_by_dimension,
                color=styles[len(valuesOfInterest) - 1]["color"],
                ls="",
                marker="x",
                markersize=20,
        ...
        if no_target_reached:
            ...
            plt.annotate(
                "no target reached", xy=(0.5, 0.5), xycoords="axes fraction", fontsize=14, horizontalalignment="center", verticalalignment="center"
            )
        plt.ylim(ylim)
    # median
    if mediandata:
        ...
        for i in displaynumber:  # display median where success prob is smaller than one
            ...
            plt.plot(
                (i,),
                (tm[1] / i**ynormalize_by_dimension,),
        ...
        if displaynumber:  # displayed only for the smallest valuesOfInterest
            for _k, j in displaynumber.items():
                ...
                plt.text(
                    j[0],
                    1.5 * j[1] / ds.dim**ynormalize_by_dimension,
                    "%.0f" % j[2],
                    axes=a,
...
def plot_previous_algorithms(func, target=None):  # lambda x: [1e-8]):
    ...
    res = plt.plot(dimensions, refalgdata, color=refcolor, linewidth=10, marker="d", markersize=25, markeredgecolor="k", zorder=-2)
    return res
...
def main(dsList, _valuesOfInterest, outputdir):
    ...
    joined_values_of_interest = (
        ", ".join(values_of_interest.labels()) if genericsettings.runlength_based_targets else ", ".join(values_of_interest.loglabels())
    )
    ...
    for ifunc, func in enumerate(dictFuncCons):
        ...
        for idim, dim in enumerate(dd.keys()):
            ...
            plt.text(loc[0], loc[1], display_text, fontsize=12, horizontalalignment=align[0], verticalalignment=align[1])
            ...
            with warnings.catch_warnings(record=True) as ws:
                ...
                if len(ws):
                    for w in ws:
                        print(w)
                    print('while saving figure in "' + filename + '" (in ppfigcons1.py:551)')
