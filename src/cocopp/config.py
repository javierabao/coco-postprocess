...
def _str_to_colormap(s, len_):
    ...
    return iter(mpl.colors.to_hex(c) for c in plt.get_cmap(cvals[0])(np.linspace(c0, c1, len_)))
...
def _index_after_float(name, return_first_digit_index=False):
    ...
    while i < len(name):
        ...
        if i == len(name):  # return is copy-paste from below
            return first_digit_index if return_first_digit_index else (last_digit_index + 1)
        if "0" <= name[i] <= "9" or (name[i] == "." and not exponent and not found[1]) or (accept_minus and name[i] == "-"):
...
def config_line_styles():
    ...
    if not settings.parameter_sweep_colormaps:
        ...
        if s.startswith(tuple(mpl.colormaps())):
            ...
            warnings.warn(
                "{0} doesn't conform with any ``matplotlib.colormaps()={1}``Hence we use the default {2}".format(
                    s, plt.colormaps(), settings.sequential_colormaps
                )
            )
            cvals = settings.sequential_colormaps
    # map algorithm argument index to first algorithm appearance index
    mapping = settings.line_style_mapping or map_indices_to_line_styles(settings._current_args)
    counts = collections.Counter(mapping.values())  # count number of appearances
    if settings.verbose >= 0:
        print("config_line_styles: found {0} distinct algorithm(s) in indices {1} of {2} arguments".format(len(counts), sorted(counts), len(mapping)))
    ...
    for alg in counts:
        ...
        settings.line_styles = sorted_line_styles(settings.line_styles, settings._current_args, indices)
...
def config(suite_name=None):
    ...
    if tbs.current_testbed and tbs.current_testbed.name not in tbs.suite_to_testbed:
        if (
            settings.isExpensive in (True, 1) or settings.runlength_based_targets in (True, 1)
        ) and tbs.current_testbed.reference_algorithm_filename == "":
            warnings.warn("Expensive setting not yet supported with " + tbs.current_testbed.name + " testbed; using non-expensive setting instead.")
    ...
    if settings.runlength_based_targets in (True, 1) and tbs.current_testbed:
        print(
            "Reference algorithm based target values, using "
        ...
        if tbs.current_testbed:
            ...
            testbed.rldValsOfInterest = pproc.RunlengthBasedTargetValues(
                settings.target_runlengths_in_single_rldistr, reference_data=reference_data, force_different_targets_factor=10**-0.2
            )

            testbed.ppfigdim_target_values = pproc.RunlengthBasedTargetValues(
                settings.target_runlengths_in_scaling_figs, reference_data=reference_data, force_different_targets_factor=10**-0.2
            )
            ...
            testbed.ppscatter_target_values = pproc.RunlengthBasedTargetValues(settings.target_runlengths_ppscatter)
            # pptable:
            testbed.pptable_targetsOfInterest = pproc.RunlengthBasedTargetValues(
                testbed.pptable_target_runlengths, reference_data=reference_data, force_different_targets_factor=10**-0.2
            )
            # pptables:
            testbed.pptablemany_targetsOfInterest = pproc.RunlengthBasedTargetValues(
                testbed.pptables_target_runlengths, reference_data=reference_data, force_different_targets_factor=10**-0.2
            )
            # ppfigs
            testbed.ppfigs_ftarget = pproc.RunlengthBasedTargetValues([settings.target_runlength], reference_data=reference_data)

        # pprldistr:
        pprldistr.runlen_xlimits_max = settings.maxevals_fix_display / 2 if settings.maxevals_fix_display else None  # can be None
