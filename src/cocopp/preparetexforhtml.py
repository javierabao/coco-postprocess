...
def main(latex_commands_for_html):
    ...
    single_objective_testbed = testbedsettings.default_suite_single_noisy if genericsettings.isNoisy else testbedsettings.default_suite_single
    ...
    for scenario in testbedsettings.all_scenarios:
        ...
        # 1. ppfigs
        f.writelines(prepare_providecommand("bbobECDFslegend", scenario, ppfigs.prepare_ecdfs_figure_caption()))
        f.writelines(prepare_providecommand("bbobppfigslegend", scenario, ppfigs.prepare_scaling_figure_caption()))

        # 2. pprldistr
        f.writelines(prepare_providecommand("bbobpprldistrlegend", scenario, pprldistr.caption_single()))
        ...
        # 3. ppfigdim
        f.writelines(
            prepare_providecommand(
                "bbobppfigdimlegend", scenario, ppfigdim.scaling_figure_caption().replace("values_of_interest", "valuesofinterest")
            )
        )
        ...
        f.writelines(
            prepare_providecommand(
                "bbobloglosstablecaption", scenario, pplogloss.table_caption().replace("Figure~\\ref{fig:ERTlogloss}", "the following figure")
            )
        )
        f.writelines(
            prepare_providecommand(
                "bbobloglossfigurecaption", scenario, pplogloss.figure_caption().replace("Figure~\\ref{tab:ERTloss}", "the previous figure")
            )
        )

        if scenario == testbedsettings.scenario_constrainedfixed:
            # 8. ppfigcons and ppfigcons1
            f.writelines(prepare_providecommand("bbobppfigconslegend", scenario, ppfigcons.prepare_scaling_figure_caption()))
            f.writelines(
                prepare_providecommand(
                    "bbobppfigconsonelegend", scenario, ppfigcons1.scaling_figure_caption().replace("values_of_interest", "valuesofinterest")
                )
            )
