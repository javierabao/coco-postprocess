    ...
    for func in dictFunc:
        ...
        for i_target in range(len(valuesOfInterest)):
            ...
            for idim, dim in enumerate(dimensions):
                if len(dictFunc[func][dim]) > 1:
                    raise ppfig.Usage("\nFound more than one algorithm inside one data folder. Specify a separate data folder for each algorithm.")
            ...
            if len(succ) > 0:
                ...
                if genericsettings.scaling_figures_with_boxes:
                    for dim in dimensions:
                        ...
                        if dictFunc[func][dim][0].detSuccesses([valuesOfInterest((func, dim))[i_target]])[0]:
                            ...
                            y = np.array(y) / dim
                            plt.plot(
                                [x[0], xm[0], x[0], x[1], xm[1], x[1], x[0]], [y[0], y[1], y[2], y[2], y[1], y[0], y[0]], markersize=0, **styles2
                            )
                            styles2["linewidth"] = 0
                ...
                for i, n in enumerate(tmp[:, 0]):
                    ...
                    if dimensions[j + 1] == tmp[i + 1, 0]:
                        res.extend(
                            plt.plot(
                                tmp[i : i + 2, 0],
                                tmp[i : i + 2, 1] / tmp[i : i + 2, 0] ** ynormalize_by_dimension,
                                markersize=0,
                                clip_on=True,
                                **styles[i_target],
                            )
                ...
                res.extend(plt.plot(tmp[:, 0], tmp[:, 1] / tmp[:, 0] ** ynormalize_by_dimension, markersize=20, clip_on=True, **styles[i_target]))
                # restore linewidth
                if lw:
        ...
        for i in reversed(range(len(valuesOfInterest))):
            res.extend(
                plt.plot(
                    ...
                    if isinstance(valuesOfInterest, pproc.RunlengthBasedTargetValues)
                    else valuesOfInterest.loglabel(i),
                    **styles[i],
                )
            )
        ...
        if 1 < 3:  # maxevals
            ...
            res.extend(
                plt.plot(
                    tmp[:, 0],
                    maxevals / tmp[:, 0] ** ynormalize_by_dimension,
                    color=styles[len(valuesOfInterest) - 1]["color"],
                    ls="",
                    marker="x",
                    markersize=20,
        ...
        if displaynumber:  # displayed only for the smallest valuesOfInterest
            for _k, j in displaynumber.items():
                ...
                plt.text(
                    j[0],
                    1.5 * j[1] / j[0] ** ynormalize_by_dimension,
                    "%.0f" % j[2],
                    axes=a,
                    horizontalalignment="center",
...
def main(dsList, _valuesOfInterest, outputdir):
    ...
    for func in dictFunc:
        ...
        if all(set(d.instancenumbers) == set(dictFunc[func][0].instancenumbers) for d in dictFunc[func]):  # all the same?
        ...
        fs_scaler = min((1, instance_text_max_len / (0.1 + max([len(t) for t in display_text.split("\n")]))))
        plt.text(
            plt.xlim()[0],
            plt.ylim()[0],
            display_text,
            fontsize=fs_scaler * instance_text_fontsize,
