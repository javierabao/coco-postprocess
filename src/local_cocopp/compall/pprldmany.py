...
)  # number of bootstrap samples drawn for each fct+target in the performance profile
nbperdecade = 1
...
def text_infigure_if_constraints():
    ...
    if testbedsettings.current_testbed.has_constraints:
        ...
        _text = "\nevals = " + ("%sx" % w[0] if w[0] != 1 else "") + "f-evals + " + ("%sx" % w[1] if w[1] != 1 else "") + "g-evals"
    else:
        _text = ""
    return _text
...
TAO = ("BFGS", "NELDER (Han)", "NEWUOA", "full NEWUOA", "BIPOP-CMA-ES", "IPOP-SEP-CMA-ES", "(1+1)-CMA-ES", "(1+1)-ES", "simple GA", "Monte Carlo")
TAOp = TAO + ("NELDER (Doe)",)
MC = ("Monte Carlo",)

third = ("POEMS", "VNS (Garcia)", "DE-PSO", "EDA-PSO", "PSO_Bounds", "PSO", "AMaLGaM IDEA", "iAMaLGaM IDEA", "MA-LS-Chain", "DASA", "BayEDAcG")
...
def plotdata(data, maxval=None, maxevals=None, CrE=0.0, maxevals2=None, **kwargs):
    ...
    if n == 0:
        ...
        res = pprldistr.plotECDF(np.array((1.0,)), n=np.inf, **kwargs)
        ...
        res = ppfig.plotUnifLogXMarkers(x2, y2, nbperdecade * 3 / np.log10(maxval), logscale=False, clip_on=False, **kwargs)
        # res = plotUnifLogXMarkers(x2, y2, nbperdecade, logscale=False, **kwargs)

        for maxeval_, format in ((maxevals, max_evals_marker_format), (maxevals2, max_evals_single_marker_format)):
            if not maxeval_:  # cover the case where maxevals is None or empty
                continue
            x3 = np.median(maxeval_)  # change it only here
            if (
                (budget_cross_always or x3 <= maxval)
                and
...
def plotLegend(handles, maxval):
    ...
    handles_with_legend = [
        h
        for h in handles_with_legend  # fix for matplotlib since v 3.5.0
    ...
    for j in sorted(ys.keys()):
        for k in reversed(sorted(ys[j].keys())):
            ...
            for h in ys[j][k]:
                if not plt.getp(h, "label").startswith("_line") and (len(show_algorithms) == 0 or plt.getp(h, "label") in show_algorithms):
                    is_recommendation = False
                    if plt.getp(h, "label") in genericsettings.foreground_algorithm_list:
                        is_recommendation = genericsettings.use_recommendations[
                            min((len(genericsettings.use_recommendations) - 1, genericsettings.foreground_algorithm_list.index(plt.getp(h, "label"))))
                        ]
                    y = 0.02 + i * 0.96 / (lh - 1)
                    tmp = {}
                    for attr in ("lw", "linestyle", "marker", "markeredgewidth", "markerfacecolor", "markeredgecolor", "markersize", "zorder"):
                    ...
                    reshandles.append(
                        plt.text(
                            maxval ** (0.02 + annotation_line_end_relative),
                            y,
    ...
    reshandles.append(plt_plot((maxval, maxval), (0.0, 1.0), color="k"))
    reslabels.reverse()
        ...
        for t in targets((entry.funcId, entry.dim)):
            ...
            if len(runlengthsucc) > 0:  # else x == [inf, inf,...]
                if testbedsettings.current_testbed.instances_are_uniform:
                    x = toolsstats.drawSP(runlengthsucc, runlengthunsucc, percentiles=[50], samplesize=perfprofsamplesize)[1]
                else:
                    nruns = len(runlengthsucc) + len(runlengthunsucc)
                    if perfprofsamplesize % nruns:
                        warnings.warn("without simulated restarts nbsamples=%d should be a multiple of nbruns=%d" % (perfprofsamplesize, nruns))
...
def all_single_functions(dict_alg, is_single_algorithm, sorted_algs=None, output_dir=".", parent_html_file_name=None, settings=genericsettings):
    single_fct_output_dir = (
        output_dir.rstrip(os.sep) + os.sep + "pprldmany-single-functions"
        # + os.sep + ('f%03d' % fg)
    )
...
def main(
    dictAlg, order=None, outputdir=".", info="default", dimension=None, parentHtmlFileName=None, plotType=PlotType.ALG, settings=genericsettings
):
    ...
    for alg in algorithms_with_data:
        CrE = 0.0
        if 1 < 3 and str(dictAlg[alg][0].algId).startswith("GLOBAL") and (not dictAlg[alg][0].indexFiles or "_pal" in dictAlg[alg][0].indexFiles[0]):
    ...
    for i, dim in enumerate(dims):
        ...
        if samplesize > 1e4:
            warntxt = "Sample size equals {} which may take very long. This is likely to be unintended, hence a bug.".format(samplesize)
            ...
            for j, t in enumerate(targets):
                ...
                for alg in algorithms_with_data:
                    ...
                    try:
                        ...
                        if len(runlengthsucc) > 0:  # else x == [inf, inf,...]
                            if testbedsettings.current_testbed.instances_are_uniform:
                                x = toolsstats.drawSP(runlengthsucc, runlengthunsucc, percentiles=[50], samplesize=samplesize)[1]
                            else:
                                nruns = len(runlengthsucc) + len(runlengthunsucc)
                                if samplesize % nruns:
                                    warnings.warn("without simulated restarts nbsamples=%d should be a multiple of nbruns=%d" % (samplesize, nruns))
    ...
    lines = []
    if displaybest:
        args = {"label": testbedsettings.current_testbed.reference_algorithm_displayname, "zorder": -1}
        args.update(genericsettings.reference_algorithm_styles)
        lines.append(plotdata(np.array(xbest), x_limit, maxevalsbest, CrE=0.0, **args))
    ...
    if styles[0]["color"] == "#000080":  # fix old styles marker size
        for s in styles:
            try:
                s["markersize"] /= 3.3  # apparently the appearance of sizes changed
            except:
                pass
    for plotting_style in plotting_style_list:
        for i, alg in enumerate(plotting_style.algorithm_list):
            ...
            args["linewidth"] *= 1 + max(
                (
                    -0.5,
                    min(
                        (
                            1.5,  # larger zorder creates thicker lines below others
                            2 - args.get("zorder", 2),
                        )
                    ),
                )
            ...
            lines.append(plotdata(np.array(data), x_limit, dictMaxEvals[alg], maxevals2=dictMaxEvals2[alg], CrE=CrEperAlg[alg], **args))
        ...
        with open(file_name, "w") as file_obj:
            ...
            if not commandnames:  # see issue #46, no data in the foreground
                ...
                if len(order) > 28:  # latex sidepanel won't work well for more than 25 algorithms, but original labels are also clipped
                    file_obj.write(r"\providecommand{\perfprofsidepanel}{\mbox{%s}\vfill\mbox{%s}}" % (commandnames[0], commandnames[-1]))
                else:
                    fontsize_command = r"\tiny{}" if len(order) > 19 else ""
                    file_obj.write(r"\providecommand{\perfprofsidepanel}{{%s\mbox{%s}" % (fontsize_command, commandnames[0]))
    ...
    if plotType == PlotType.FUNC:
        ...
        text = "%s\n%s, %d-D" % (testbedsettings.current_testbed.name, groupName, dimList[0])
    else:
        text = "%s %s" % (testbedsettings.current_testbed.name, ppfig.consecutiveNumbers(sorted(dictFunc.keys()), "f"))
    ...
    if isinstance(target_values, pp.RunlengthBasedTargetValues):
        text += str(len(targetstrings)) + " targets RLs/dim: " + targetstrings[0] + ".." + targetstrings[len(targetstrings) - 1] + "\n"
        text += "  from " + testbedsettings.current_testbed.reference_algorithm_filename
    else:
        text += str(len(targetstrings)) + " targets: " + targetstrings[0] + ".." + targetstrings[len(targetstrings) - 1]
    ...
    if len(dictFunc) == 1:
        plt.title(
            " ".join((str(list(dictFunc.keys())[0]), testbedsettings.current_testbed.short_names[list(dictFunc.keys())[0]])), fontsize=title_fontsize
        )
    a = plt.gca()
    ...
    a.set_xticks(
        [
            i * 10**d
            for d in range(log_xlimit)  # these should be default
            for i in [2, 3, 4, 5, 6, 7, 8, 9]
        ],
        minor=True,
    )
    a.set_yticks(np.linspace(0, 1, 21), minor=True)  # every 0.05 = 5%
    a.tick_params(top=True, bottom=True, which="both", left=True, right=True, direction="out")
    a.grid(True)
    a.grid(genericsettings.minor_grid_alpha_in_pprldmany > 0, alpha=genericsettings.minor_grid_alpha_in_pprldmany, which="minor")
