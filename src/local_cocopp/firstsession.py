...
dataurl = "https://numbbo.github.io/data-archive/data-archive/bbob/2009/BIPOP-CMA-ES_hansen_noiseless.tgz"
filename, headers = urlretrieve(dataurl)
archivefile = tarfile.open(filename)
archivefile.extractall()

# Display some information
ds = local_cocopp.load("BIPOP-CMA-ES/bbobexp_f2.info")
print(ds)
...
# Load a pickle file
ds = local_cocopp.load("BBOB2009pythondata/BIPOP-CMA-ES/ppdata_f002_20.pickle")
# Load a folder
ds = local_cocopp.load("BIPOP-CMA-ES")

# Load data using a wildcard
import glob

ds = local_cocopp.load(glob.glob("BIPOP-CMA-ES/ppdata_f002_*.pickle"))

# Display function values versus time
ds = local_cocopp.load("BBOB2009pythondata/BIPOP-CMA-ES/ppdata_f002_20.pickle")
local_cocopp.info(ds)  # display information on DataSetList ds
...
loglog(budgets, median(funvals, axis=1), linewidth=3, color="r", label="median")
...
ds1 = local_cocopp.load("BBOB2009pythondata/NEWUOA/ppdata_f002_20.pickle")
print(ds1)
d1 = ds1[0]
...
loglog(budgets1, median(funvals1, axis=1), linewidth=3, color="g", label="median NEWUOA")
...
loglog(d.target[d.target >= 1e-8], d.ert[d.target >= 1e-8], lw=3, color="r", label="ert")
...
ylabel("Targets")
loglog(d.ert[d.target >= 1e-8], d.target[d.target >= 1e-8], lw=3, color="r", label="ert")
legend()
...
ds = local_cocopp.load(glob.glob("BBOB2009pythondata/BIPOP-CMA-ES/ppdata_f0*_20.pickle"))
figure()
pprldistr.plot(ds)
...
ds = local_cocopp.load(glob.glob("BBOB2009pythondata/BIPOP-CMA-ES/ppdata_f002_*.pickle"))
figure()
ppfigdim.plot(ds)
ppfigdim.beautify()
