...
savepath = "local_cocopp/pprldistr2009_hardestRLB.pickle"
import pickle
import local_cocopp
import numpy as np
...
for datapath in datapaths:
    ...
    data2009 = local_cocopp.load(datapath)
    Algs = data2009.dictByAlg()
    target_runlengths_in_table = [0.5, 1.2, 3, 10, 50]
    targets = local_cocopp.pproc.RunlengthBasedTargetValues(target_runlengths_in_table, force_different_targets_factor=10**-0.2)
    ...
    for alg in Algs:
        ...
        for func in curAlg:
            ...
            for dim in funcdata:
                ...
                local_cocopp.pprldistr.plotECDF(x[np.isfinite(x)] / float(dim), len(y))
        print(algname, "done")
