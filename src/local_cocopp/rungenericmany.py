...
def main(args, outputdir):
    ...
    for i, alg in enumerate(algs):
        lines.append("\\providecommand{\\algorithm" + pptex.numtotext(i) + "}{" + str_to_latex(strip_pathname1(alg)) + "}")
    prepend_to_file(latex_commands_file, lines, 5000, "bbob_proc_commands.tex truncated, consider removing " + "the file before the text run")
        ...
        for i in dsList:
            ...
            for instance_set_of_interest in genericsettings.instancesOfInterest:
                if curr_instances == instance_set_of_interest:
                    correct = True
            try:
                last_incorrect_instances  # noqa: F821
            except:
                last_incorrect_instances = None
            if not correct and curr_instances != last_incorrect_instances:
                warnings.warn(
                    "The data of %s " % i + "(and possibly further data) do not " + "list the correct instances " + "of function F%d." % i.funcId
                )
    ...
    if genericsettings.isRLDistr:
        ...
        if len(genericsettings.foreground_algorithm_list) == 2:
            ...
            if testbedsettings.current_testbed not in [testbedsettings.GECCOBiObjBBOBTestbed, testbedsettings.GECCOBiObjExtBBOBTestbed]:
        ...
        if genericsettings.isRldOnSingleFcts:
            ...
            if 1 < 3:
                pprldmany.all_single_functions(
                    dictAlg, False, sortedAlgs, many_algorithms_output, genericsettings.many_algorithm_file_name, settings=genericsettings
                )
                ...
                for fg, tmpdictAlg in dictFG.items():
                    ...
                    for i, d in enumerate(dims):
                        ...
                        single_fct_output_dir = (
                            many_algorithms_output.rstrip(os.sep) + os.sep + "pprldmany-single-functions"
                            # + os.sep + ('f%03d' % fg)
                        )
    ...
    if genericsettings.isTab:
        ...
        prepend_to_file(latex_commands_file, [r"\providecommand{\bbobpptablesmanylegend}[1]{" + pptables.get_table_caption() + "}"])
        ...
        for ng, tmpdictng in dictNoi.items():
            ...
            for d, tmpdictdim in sorted(dictDim.items()):
                pptables.main(
                    ...
                    sortedAlgs,
                    many_algorithms_output,
                    ([1, 20, 38] if (testbedsettings.current_testbed.name == testbedsettings.suite_name_bi) else True),
                    latex_commands_file,
                )
        print_done()
    ...
    if genericsettings.isScatter and len(genericsettings.foreground_algorithm_list) == 2:
        ...
        prepend_to_file(latex_commands_file, ["\\providecommand{\\bbobppscatterlegend}[1]{", ppscatter.figure_caption(), "}"])
    ...
    if genericsettings.isFig:
        ...
        ppfigs.main(dictAlg, genericsettings.ppfigs_file_name, sortedAlgs, many_algorithms_output, latex_commands_file)
        print_done()

    if testbedsettings.current_testbed.has_constraints:
        print("Scaling wrt constraints...")
        ppfigcons.main(dictAlg, genericsettings.ppfigcons_file_name, sortedAlgs, many_algorithms_output, latex_commands_file)
        print_done()
